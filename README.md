...
| 📁 **磁盘分析** | Treemap 矩形树图可视化目录空间占用；矩形大小直接反映文件/文件夹大小；点击钻入子目录；面包屑导航；自动扫描用户主目录 |
...
│   ├── disk_analyzer.py #   磁盘分析 — Treemap 可视化目录空间分布
│   └── settings.py      #   设置 — 主题切换
├── core/                # 核心逻辑层（无 GUI 依赖）
...
│   ├── disk_analyzer.py #   磁盘扫描器（目录递归扫描 + 大小统计）
...
- **[squarify](https://github.com/uri-nyx/squarify)** — Treemap 矩形树图布局
